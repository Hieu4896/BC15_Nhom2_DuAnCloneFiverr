...
import Style from "./loaiCongViecIphone.module.css";
import { useState } from "react";
import { NavLink } from "react-router-dom/cjs/react-router-dom.min";
export default function LoaiCongViecIphone(props) {
    ...
    return congViecTheoLoaiCongViec.map((job, index) => {
      return (
        <div className="col-12" key={index}>
          ...
          <p style={{ fontSize: 15, marginTop: "10px" }}>{job.name}</p>
        </div>
      );
    });
  };
    ...
    <div style={{ position: "relative" }}>
      {loading ? (
        <div style={{ padding: "0 50px" }}>
          <h2 style={{ textAlign: "center", padding: "20px 0", fontSize: 24 }}>
            {loaiCongViec.name}
          </h2>
          <div className="row">
            <div className="col-4">
